...
def make_gfp_dCas9_module(vector: sbol3.Feature) -> Tuple[sbol3.Feature]:
    """Add a module with GFP and dCas9 expression to the system

    Arguments:
        vector (sbol3.Feature): Vector into which the coding materials for the
            CRISPR module will be added

    Returns:
        dCas9 (sbol3.LocalSubComponent): Protein molecule for the gRNAs to form
            complexes with
        gfp_cds (sbol3.LocalSubComponent): Coding sequence for GFP to attach
            regulation to
    """
    # Find the system containing the vector
    ...
    # dCas9 #
    #########
    # Add constitutive expression of dCas9
    # FIXME: Should this be specific to the different versions of dCas9 (i.e.
    # R, V2?)
    dCas9_cds = contains(vector,
                         sbol3.LocalSubComponent([sbol3.SBO_DNA],
                                                 roles=[tyto.SO.CDS],
                                                 name="dCas9-coding"))
    constitutive(dCas9_cds)
    dCas9 = add_feature(system,
                        sbol3.LocalSubComponent([sbol3.SBO_PROTEIN],
                                                name="dCas9"))
    add_interaction(sbol3.SBO_GENETIC_PRODUCTION,
                    {dCas9_cds: sbol3.SBO_TEMPLATE, dCas9: sbol3.SBO_PRODUCT})
    add_interaction(sbol3.SBO_DEGRADATION,
                    name='dCas9 degradation',
                    participants={dCas9: sbol3.SBO_REACTANT})

    ###########################################################################
    # GFP
    # Note: This is only constitutive expression, gRNA regulation is added
    # later
    ###########################################################################
    # Create GFP's promoter as part of the vector
    gfp_promoter = contains(vector, sbol3.LocalSubComponent([sbol3.SBO_DNA],
                            roles=[tyto.SO.promoter]))
    # Create GFP's coding region
    gfp_cds = contains(vector,
                       sbol3.LocalSubComponent([sbol3.SBO_DNA],
                                               roles=[tyto.SO.CDS],
                                               name="GFP-coding"))
    # Add GFP protein to the system
    gfp = add_feature(system,
                      sbol3.LocalSubComponent([sbol3.SBO_PROTEIN],
                                              name="GFP"))
    # Make the promoter regulate the GFP coding sequence
    regulate(gfp_promoter, gfp_cds)
    # Use the coding region to produce the protein
    add_interaction(sbol3.SBO_GENETIC_PRODUCTION,
                    {gfp_cds: sbol3.SBO_TEMPLATE, gfp: sbol3.SBO_PRODUCT})
    # Add protein degradation
    add_interaction(sbol3.SBO_DEGRADATION,
                    name='GFP degradation',
                    participants={gfp: sbol3.SBO_REACTANT})

    # The dCas9 protein needs to be returned so that it can be used to interact
    # with the gRNA and form a complex
    # The promoter for GFP needs to be returned so that it can have
    # interactions with the dCas9-gRNA complexes
    return dCas9, gfp_promoter, gfp


def make_gRNA_module(vector: sbol3.Feature,
                     dCas9: sbol3.Feature,
                     promoter: sbol3.Feature,
                     repressor: bool,
                     idx_gRNA: int = 1,
                     n_interactions: int = 1):
    """Add a module with gRNA expression for one gRNA to the system

    Arguments:
        vector (sbol3.Feature): Vector into which the coding materials for the
            gRNA module will be added
        dCas9 (sbol3.Feature): dCas9 protein to which the gRNAs will bind
        promoter (sbol3.Feature): Promoter that the gRNA targets
        repressor (bool): true for repressor, false for activator
        idx_gRNA (int): Index number for the gRNA (i.e. 1 for a single gRNA
            system, 1+ for a multiplex system)
        n_interactions (int): The number of interactions to add between the
            gRNA complex and the target promoter. Equivalent to the number of
            target sites in the promoter.

    Returns:
        dCas9_gRNA (sbol3.Feature): Non-covalent complex of dCas9 protein and
            gRNA
    """
    # Check that the index makes sense (i.e. not 0 or negative)
    if not idx_gRNA > 0:
        raise ValueError(f'Index should be > 0 but was not: {idx_gRNA}')
    ...
    # Add constitutive expression of gRNA
    gRNA_cds = contains(vector,
                        sbol3.LocalSubComponent([sbol3.SBO_DNA],
                                                roles=[tyto.SO.CDS],
                                                name="gRNA" + str(idx_gRNA)
                                                + "-coding"))
    constitutive(gRNA_cds)
    gRNA = add_feature(system,
                       sbol3.LocalSubComponent([sbol3.SBO_RNA],
                                               name="gRNA" + str(idx_gRNA)))
    add_interaction(sbol3.SBO_GENETIC_PRODUCTION,
                    {gRNA_cds: sbol3.SBO_TEMPLATE, gRNA: sbol3.SBO_PRODUCT})
    add_interaction(sbol3.SBO_DEGRADATION,
                    name="gRNA" + str(idx_gRNA) + " degradation",
                    participants={gRNA: sbol3.SBO_REACTANT})

    # Add interaction of gRNA with dCas9
    dCas9_gRNA = add_feature(system,
                             sbol3.LocalSubComponent(
                                [sbol3.SBO_NON_COVALENT_COMPLEX],
                                name="dCas9-gRNA" + str(idx_gRNA)))
    add_interaction(sbol3.SBO_NON_COVALENT_BINDING,
                    name='Cas-gRNA binding',
                    participants={gRNA: sbol3.SBO_REACTANT,
                                  dCas9: sbol3.SBO_REACTANT,
                                  dCas9_gRNA: sbol3.SBO_PRODUCT})
    add_interaction(sbol3.SBO_DEGRADATION,
                    name="dCas9-gRNA" + str(idx_gRNA) + "-complex-degradation",
                    participants={dCas9_gRNA: sbol3.SBO_REACTANT})

    # Make a promoter that the gRNA regulates
    for i in range(n_interactions):
        if repressor:
            add_interaction(sbol3.SBO_INHIBITION,
                            name=f'CRISPR Interference Target {i}',
                            participants={dCas9_gRNA: sbol3.SBO_INHIBITOR,
                                          promoter: sbol3.SBO_INHIBITED})
        else:
            add_interaction(sbol3.SBO_STIMULATION,
                            name=f'CRISPR Activation Target {i}',
                            participants={dCas9_gRNA: sbol3.SBO_STIMULATOR,
                                          promoter: sbol3.SBO_STIMULATED})
...
def add_complex_interference(regulator: sbol3.feature, interferers: list):
    """Add interference reaction between the given gRNAs

    :param regulator: SBOL feature for the the "main" regulator
    :param interferers: list of SBOL features that interfere with the main
        regulator
    """
    for interfering_regulator in interferers:
        add_interaction(sbol3.SBO_CONTROL,  # TODO: Check with Jake that this is a good interaction type to use
                        name=(f'Complex Interference ({regulator.name} & '
                              f'{interfering_regulator.name})'),
                        participants={regulator: sbol3.SBO_MODIFIED,
                                      interfering_regulator: sbol3.SBO_MODIFIER})
